...
class GetMultipleCharacterUseCase @Inject constructor(private val repository: RickAndMortyRepository) {
    ...
    operator fun invoke(ids: List<Int>) : Flow<NetworkResponse<List<CharacterListItem>>> = flow {
        Log.e(TAG, "invoke was called")
        emit(NetworkResponse.Loading)
        when(val response=repository.getMultipleCharacters(ids)){
            is NetworkResponse.Error -> emit(response)
            is NetworkResponse.Loading -> Unit
            is NetworkResponse.Success -> emit(NetworkResponse.Success(response.result!!.map { it.toCharacterListItem() }))
        }
    }
}